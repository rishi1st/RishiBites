const resturants = [
          {
                    "image": "six",
                    "name": "Tandoori Tales",
                    "rating": "3.2",
                    "food_type": "Mexican",
                    "price_for_two": 212,
                    "location": "Patliputra Colony",
                    "distance": "8.4",
                    "offer": 17,
                    "alcohol": false,
                    "restaurant_opening_time": 3,
                    "restaurant_closing_time": 15
          },
          {
                    "image": "ten",
                    "name": "Patna Bites",
                    "rating": "1.1",
                    "food_type": "Fast Food",
                    "price_for_two": 2391,
                    "location": "Patliputra Colony",
                    "distance": "8.0",
                    "offer": 15,
                    "alcohol": false,
                    "restaurant_opening_time": 12,
                    "restaurant_closing_time": 0
          },
          {
                    "image": "three",
                    "name": "Grill House",
                    "rating": "1.3",
                    "food_type": "South Indian",
                    "price_for_two": 1334,
                    "location": "Boring Road",
                    "distance": "5.8",
                    "offer": 22,
                    "alcohol": false,
                    "restaurant_opening_time": 17,
                    "restaurant_closing_time": 5
          },
          {
                    "image": "five",
                    "name": "Street Eats",
                    "rating": "3.8",
                    "food_type": "Mughlai",
                    "price_for_two": 434,
                    "location": "Kankarbagh",
                    "distance": "3.2",
                    "offer": 24,
                    "alcohol": false,
                    "restaurant_opening_time": 12,
                    "restaurant_closing_time": 0
          },
          {
                    "image": "eleven",
                    "name": "Taste of Punjab",
                    "rating": "2.2",
                    "food_type": "Mexican",
                    "price_for_two": 1974,
                    "location": "Ashok Rajpath",
                    "distance": "7.6",
                    "offer": 3,
                    "alcohol": false,
                    "restaurant_opening_time": 2,
                    "restaurant_closing_time": 14
          },
          {
                    "image": "eleven",
                    "name": "The Curry Bowl",
                    "rating": "2.0",
                    "food_type": "Italian",
                    "price_for_two": 581,
                    "location": "Nala Road",
                    "distance": "7.2",
                    "offer": 26,
                    "alcohol": false,
                    "restaurant_opening_time": 4,
                    "restaurant_closing_time": 16
          },
          {
                    "image": "eight",
                    "name": "Patna Bites",
                    "rating": "2.4",
                    "food_type": "Continental",
                    "price_for_two": 1996,
                    "location": "Kurji",
                    "distance": "2.0",
                    "offer": 25,
                    "alcohol": false,
                    "restaurant_opening_time": 19,
                    "restaurant_closing_time": 7
          },
          {
                    "image": "thirteen",
                    "name": "Urban Zayka",
                    "rating": "4.2",
                    "food_type": "North Indian",
                    "price_for_two": 348,
                    "location": "Danapur",
                    "distance": "2.6",
                    "offer": 20,
                    "alcohol": true,
                    "restaurant_opening_time": 11,
                    "restaurant_closing_time": 23
          },
          {
                    "image": "five",
                    "name": "Sizzlers Spot",
                    "rating": "3.6",
                    "food_type": "Fast Food",
                    "price_for_two": 2194,
                    "location": "SP Verma Road",
                    "distance": "2.8",
                    "offer": 9,
                    "alcohol": false,
                    "restaurant_opening_time": 4,
                    "restaurant_closing_time": 16
          },
          {
                    "image": "thirteen",
                    "name": "Grill House",
                    "rating": "4.8",
                    "food_type": "South Indian",
                    "price_for_two": 1007,
                    "location": "Danapur",
                    "distance": "3.9",
                    "offer": 6,
                    "alcohol": true,
                    "restaurant_opening_time": 12,
                    "restaurant_closing_time": 0
          },
          {
                    "image": "four",
                    "name": "Urban Zayka",
                    "rating": "2.5",
                    "food_type": "Fast Food",
                    "price_for_two": 162,
                    "location": "Danapur",
                    "distance": "9.2",
                    "offer": 15,
                    "alcohol": false,
                    "restaurant_opening_time": 14,
                    "restaurant_closing_time": 2
          },
          {
                    "image": "seven",
                    "name": "Sizzlers Spot",
                    "rating": "2.9",
                    "food_type": "Mughlai",
                    "price_for_two": 184,
                    "location": "Digha",
                    "distance": "3.1",
                    "offer": 0,
                    "alcohol": true,
                    "restaurant_opening_time": 10,
                    "restaurant_closing_time": 22
          },
          {
                    "image": "eleven",
                    "name": "Street Eats",
                    "rating": "1.5",
                    "food_type": "Mexican",
                    "price_for_two": 777,
                    "location": "Bankipore",
                    "distance": "2.9",
                    "offer": 10,
                    "alcohol": false,
                    "restaurant_opening_time": 6,
                    "restaurant_closing_time": 18
          },
          {
                    "image": "six",
                    "name": "Cafe Bliss",
                    "rating": "3.0",
                    "food_type": "Thai",
                    "price_for_two": 762,
                    "location": "Rajendra Nagar",
                    "distance": "7.0",
                    "offer": 29,
                    "alcohol": false,
                    "restaurant_opening_time": 11,
                    "restaurant_closing_time": 23
          },
          {
                    "image": "five",
                    "name": "Sizzlers Spot",
                    "rating": "4.3",
                    "food_type": "Chinese",
                    "price_for_two": 737,
                    "location": "Exhibition Road",
                    "distance": "4.7",
                    "offer": 4,
                    "alcohol": false,
                    "restaurant_opening_time": 3,
                    "restaurant_closing_time": 15
          },
          {
                    "image": "two",
                    "name": "Hunger Hub",
                    "rating": "2.9",
                    "food_type": "Mexican",
                    "price_for_two": 1990,
                    "location": "Ashok Rajpath",
                    "distance": "3.3",
                    "offer": 23,
                    "alcohol": true,
                    "restaurant_opening_time": 1,
                    "restaurant_closing_time": 13
          },
          {
                    "image": "six",
                    "name": "The Curry Bowl",
                    "rating": "1.9",
                    "food_type": "North Indian",
                    "price_for_two": 468,
                    "location": "Rajendra Nagar",
                    "distance": "7.2",
                    "offer": 22,
                    "alcohol": false,
                    "restaurant_opening_time": 1,
                    "restaurant_closing_time": 13
          },
          {
                    "image": "two",
                    "name": "Biryani Express",
                    "rating": "4.3",
                    "food_type": "Italian",
                    "price_for_two": 2210,
                    "location": "Kankarbagh",
                    "distance": "8.9",
                    "offer": 25,
                    "alcohol": false,
                    "restaurant_opening_time": 1,
                    "restaurant_closing_time": 13
          },
          {
                    "image": "ten",
                    "name": "Hunger Hub",
                    "rating": "1.3",
                    "food_type": "Mexican",
                    "price_for_two": 1896,
                    "location": "S.K. Puri",
                    "distance": "8.7",
                    "offer": 17,
                    "alcohol": true,
                    "restaurant_opening_time": 10,
                    "restaurant_closing_time": 22
          },
          {
                    "image": "seven",
                    "name": "Spice Junction",
                    "rating": "2.1",
                    "food_type": "Mughlai",
                    "price_for_two": 905,
                    "location": "Danapur",
                    "distance": "2.3",
                    "offer": 25,
                    "alcohol": false,
                    "restaurant_opening_time": 4,
                    "restaurant_closing_time": 16
          },
          {
                    "image": "five",
                    "name": "Chinese Delight",
                    "rating": "3.9",
                    "food_type": "Continental",
                    "price_for_two": 1145,
                    "location": "Exhibition Road",
                    "distance": "1.9",
                    "offer": 23,
                    "alcohol": false,
                    "restaurant_opening_time": 19,
                    "restaurant_closing_time": 7
          },
          {
                    "image": "five",
                    "name": "Chinese Delight",
                    "rating": "1.2",
                    "food_type": "Chinese",
                    "price_for_two": 2079,
                    "location": "Rajendra Nagar",
                    "distance": "9.3",
                    "offer": 23,
                    "alcohol": false,
                    "restaurant_opening_time": 19,
                    "restaurant_closing_time": 7
          },
          {
                    "image": "three",
                    "name": "Patna Bites",
                    "rating": "3.2",
                    "food_type": "North Indian",
                    "price_for_two": 694,
                    "location": "Bailey Road",
                    "distance": "9.3",
                    "offer": 24,
                    "alcohol": false,
                    "restaurant_opening_time": 20,
                    "restaurant_closing_time": 8
          },
          {
                    "image": "fourteen",
                    "name": "Tandoori Tales",
                    "rating": "1.5",
                    "food_type": "Mughlai",
                    "price_for_two": 2487,
                    "location": "Exhibition Road",
                    "distance": "5.6",
                    "offer": 9,
                    "alcohol": false,
                    "restaurant_opening_time": 10,
                    "restaurant_closing_time": 22
          },
          {
                    "image": "thirteen",
                    "name": "Flavors of India",
                    "rating": "3.4",
                    "food_type": "Street Food",
                    "price_for_two": 1531,
                    "location": "SP Verma Road",
                    "distance": "6.1",
                    "offer": 27,
                    "alcohol": false,
                    "restaurant_opening_time": 11,
                    "restaurant_closing_time": 23
          },
          {
                    "image": "eight",
                    "name": "Sizzlers Spot",
                    "rating": "1.0",
                    "food_type": "Mughlai",
                    "price_for_two": 376,
                    "location": "Patna City",
                    "distance": "9.4",
                    "offer": 27,
                    "alcohol": true,
                    "restaurant_opening_time": 10,
                    "restaurant_closing_time": 22
          },
          {
                    "image": "thirteen",
                    "name": "Taste of Punjab",
                    "rating": "3.8",
                    "food_type": "Mexican",
                    "price_for_two": 1617,
                    "location": "Exhibition Road",
                    "distance": "1.2",
                    "offer": 5,
                    "alcohol": true,
                    "restaurant_opening_time": 15,
                    "restaurant_closing_time": 3
          },
          {
                    "image": "ten",
                    "name": "Pizza Mania",
                    "rating": "1.7",
                    "food_type": "Thai",
                    "price_for_two": 2244,
                    "location": "Danapur",
                    "distance": "4.0",
                    "offer": 11,
                    "alcohol": false,
                    "restaurant_opening_time": 10,
                    "restaurant_closing_time": 22
          },
          {
                    "image": "one",
                    "name": "Street Eats",
                    "rating": "2.0",
                    "food_type": "Mughlai",
                    "price_for_two": 747,
                    "location": "Fraser Road",
                    "distance": "1.6",
                    "offer": 25,
                    "alcohol": true,
                    "restaurant_opening_time": 7,
                    "restaurant_closing_time": 19
          },
          {
                    "image": "fourteen",
                    "name": "Chinese Delight",
                    "rating": "2.5",
                    "food_type": "Italian",
                    "price_for_two": 1884,
                    "location": "Ashok Rajpath",
                    "distance": "5.7",
                    "offer": 9,
                    "alcohol": false,
                    "restaurant_opening_time": 20,
                    "restaurant_closing_time": 8
          },
          {
                    "image": "ten",
                    "name": "Tandoori Tales",
                    "rating": "2.2",
                    "food_type": "Mexican",
                    "price_for_two": 1413,
                    "location": "Rajiv Nagar",
                    "distance": "1.1",
                    "offer": 4,
                    "alcohol": false,
                    "restaurant_opening_time": 9,
                    "restaurant_closing_time": 21
          },
          {
                    "image": "three",
                    "name": "Spice Junction",
                    "rating": "1.7",
                    "food_type": "Street Food",
                    "price_for_two": 228,
                    "location": "Phulwari Sharif",
                    "distance": "8.9",
                    "offer": 10,
                    "alcohol": false,
                    "restaurant_opening_time": 15,
                    "restaurant_closing_time": 3
          },
          {
                    "image": "fifteen",
                    "name": "Sizzlers Spot",
                    "rating": "2.6",
                    "food_type": "Mexican",
                    "price_for_two": 1264,
                    "location": "Patna City",
                    "distance": "8.5",
                    "offer": 24,
                    "alcohol": false,
                    "restaurant_opening_time": 2,
                    "restaurant_closing_time": 14
          },
          {
                    "image": "nine",
                    "name": "Sizzlers Spot",
                    "rating": "1.6",
                    "food_type": "Italian",
                    "price_for_two": 910,
                    "location": "Digha",
                    "distance": "4.6",
                    "offer": 26,
                    "alcohol": false,
                    "restaurant_opening_time": 15,
                    "restaurant_closing_time": 3
          },
          {
                    "image": "fifteen",
                    "name": "Cafe Bliss",
                    "rating": "1.0",
                    "food_type": "Italian",
                    "price_for_two": 1577,
                    "location": "Kurji",
                    "distance": "6.0",
                    "offer": 27,
                    "alcohol": true,
                    "restaurant_opening_time": 9,
                    "restaurant_closing_time": 21
          },
          {
                    "image": "fifteen",
                    "name": "Chinese Delight",
                    "rating": "4.5",
                    "food_type": "North Indian",
                    "price_for_two": 513,
                    "location": "Gandhi Maidan",
                    "distance": "9.4",
                    "offer": 12,
                    "alcohol": true,
                    "restaurant_opening_time": 13,
                    "restaurant_closing_time": 1
          },
          {
                    "image": "four",
                    "name": "Patna Bites",
                    "rating": "4.2",
                    "food_type": "Italian",
                    "price_for_two": 2255,
                    "location": "Gandhi Maidan",
                    "distance": "2.0",
                    "offer": 13,
                    "alcohol": true,
                    "restaurant_opening_time": 8,
                    "restaurant_closing_time": 20
          },
          {
                    "image": "seven",
                    "name": "Sizzlers Spot",
                    "rating": "3.8",
                    "food_type": "Italian",
                    "price_for_two": 249,
                    "location": "Phulwari Sharif",
                    "distance": "7.2",
                    "offer": 29,
                    "alcohol": false,
                    "restaurant_opening_time": 21,
                    "restaurant_closing_time": 9
          },
          {
                    "image": "eight",
                    "name": "Street Eats",
                    "rating": "1.2",
                    "food_type": "Thai",
                    "price_for_two": 1829,
                    "location": "Danapur",
                    "distance": "4.3",
                    "offer": 26,
                    "alcohol": false,
                    "restaurant_opening_time": 20,
                    "restaurant_closing_time": 8
          },
          {
                    "image": "one",
                    "name": "Chow Down",
                    "rating": "2.5",
                    "food_type": "Mughlai",
                    "price_for_two": 506,
                    "location": "Patliputra Colony",
                    "distance": "2.3",
                    "offer": 2,
                    "alcohol": true,
                    "restaurant_opening_time": 1,
                    "restaurant_closing_time": 13
          },
          {
                    "image": "fifteen",
                    "name": "Biryani Express",
                    "rating": "4.7",
                    "food_type": "Mexican",
                    "price_for_two": 1124,
                    "location": "Anisabad",
                    "distance": "9.5",
                    "offer": 3,
                    "alcohol": true,
                    "restaurant_opening_time": 14,
                    "restaurant_closing_time": 2
          },
          {
                    "image": "three",
                    "name": "Street Eats",
                    "rating": "3.2",
                    "food_type": "Chinese",
                    "price_for_two": 700,
                    "location": "Ashok Rajpath",
                    "distance": "7.4",
                    "offer": 12,
                    "alcohol": false,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "six",
                    "name": "Chinese Delight",
                    "rating": "4.8",
                    "food_type": "North Indian",
                    "price_for_two": 1415,
                    "location": "Rajendra Nagar",
                    "distance": "3.9",
                    "offer": 15,
                    "alcohol": true,
                    "restaurant_opening_time": 3,
                    "restaurant_closing_time": 15
          },
          {
                    "image": "thirteen",
                    "name": "Fusion Diner",
                    "rating": "4.2",
                    "food_type": "Continental",
                    "price_for_two": 1694,
                    "location": "Danapur",
                    "distance": "7.3",
                    "offer": 23,
                    "alcohol": false,
                    "restaurant_opening_time": 14,
                    "restaurant_closing_time": 2
          },
          {
                    "image": "two",
                    "name": "Tandoori Tales",
                    "rating": "1.9",
                    "food_type": "Fast Food",
                    "price_for_two": 1010,
                    "location": "Gandhi Maidan",
                    "distance": "8.4",
                    "offer": 16,
                    "alcohol": true,
                    "restaurant_opening_time": 0,
                    "restaurant_closing_time": 12
          },
          {
                    "image": "twelve",
                    "name": "Biryani Express",
                    "rating": "4.8",
                    "food_type": "Chinese",
                    "price_for_two": 2316,
                    "location": "Nala Road",
                    "distance": "1.7",
                    "offer": 1,
                    "alcohol": false,
                    "restaurant_opening_time": 12,
                    "restaurant_closing_time": 0
          },
          {
                    "image": "five",
                    "name": "Tandoori Tales",
                    "rating": "3.9",
                    "food_type": "Street Food",
                    "price_for_two": 1065,
                    "location": "Rajendra Nagar",
                    "distance": "9.4",
                    "offer": 22,
                    "alcohol": false,
                    "restaurant_opening_time": 10,
                    "restaurant_closing_time": 22
          },
          {
                    "image": "fourteen",
                    "name": "Sizzlers Spot",
                    "rating": "2.8",
                    "food_type": "Mexican",
                    "price_for_two": 1555,
                    "location": "SP Verma Road",
                    "distance": "7.0",
                    "offer": 27,
                    "alcohol": true,
                    "restaurant_opening_time": 12,
                    "restaurant_closing_time": 0
          },
          {
                    "image": "seven",
                    "name": "Chow Down",
                    "rating": "4.4",
                    "food_type": "Thai",
                    "price_for_two": 740,
                    "location": "Nala Road",
                    "distance": "2.4",
                    "offer": 15,
                    "alcohol": false,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "eight",
                    "name": "Sizzlers Spot",
                    "rating": "2.2",
                    "food_type": "Italian",
                    "price_for_two": 357,
                    "location": "Nala Road",
                    "distance": "3.5",
                    "offer": 28,
                    "alcohol": false,
                    "restaurant_opening_time": 7,
                    "restaurant_closing_time": 19
          },
          {
                    "image": "eleven",
                    "name": "Chinese Delight",
                    "rating": "3.2",
                    "food_type": "Chinese",
                    "price_for_two": 2179,
                    "location": "Rajiv Nagar",
                    "distance": "9.3",
                    "offer": 6,
                    "alcohol": false,
                    "restaurant_opening_time": 3,
                    "restaurant_closing_time": 15
          },
          {
                    "image": "one",
                    "name": "Tandoori Tales",
                    "rating": "3.7",
                    "food_type": "Mexican",
                    "price_for_two": 1762,
                    "location": "Patna City",
                    "distance": "1.9",
                    "offer": 20,
                    "alcohol": false,
                    "restaurant_opening_time": 9,
                    "restaurant_closing_time": 21
          },
          {
                    "image": "one",
                    "name": "Patna Bites",
                    "rating": "4.8",
                    "food_type": "Fast Food",
                    "price_for_two": 1888,
                    "location": "Bankipore",
                    "distance": "6.8",
                    "offer": 7,
                    "alcohol": false,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "ten",
                    "name": "Cafe Bliss",
                    "rating": "4.2",
                    "food_type": "Street Food",
                    "price_for_two": 1066,
                    "location": "Nala Road",
                    "distance": "8.1",
                    "offer": 6,
                    "alcohol": false,
                    "restaurant_opening_time": 20,
                    "restaurant_closing_time": 8
          },
          {
                    "image": "three",
                    "name": "Fusion Diner",
                    "rating": "1.4",
                    "food_type": "Street Food",
                    "price_for_two": 1617,
                    "location": "SP Verma Road",
                    "distance": "8.0",
                    "offer": 2,
                    "alcohol": true,
                    "restaurant_opening_time": 0,
                    "restaurant_closing_time": 12
          },
          {
                    "image": "twelve",
                    "name": "The Food Factory",
                    "rating": "1.6",
                    "food_type": "Thai",
                    "price_for_two": 306,
                    "location": "Rajendra Nagar",
                    "distance": "3.1",
                    "offer": 8,
                    "alcohol": true,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "four",
                    "name": "Royal Feast",
                    "rating": "3.7",
                    "food_type": "Mexican",
                    "price_for_two": 972,
                    "location": "Digha",
                    "distance": "1.3",
                    "offer": 2,
                    "alcohol": false,
                    "restaurant_opening_time": 0,
                    "restaurant_closing_time": 12
          },
          {
                    "image": "nine",
                    "name": "Sizzlers Spot",
                    "rating": "3.0",
                    "food_type": "Chinese",
                    "price_for_two": 2012,
                    "location": "Bailey Road",
                    "distance": "7.0",
                    "offer": 10,
                    "alcohol": false,
                    "restaurant_opening_time": 15,
                    "restaurant_closing_time": 3
          },
          {
                    "image": "eleven",
                    "name": "Flavors of India",
                    "rating": "3.9",
                    "food_type": "Mexican",
                    "price_for_two": 649,
                    "location": "Ashok Rajpath",
                    "distance": "4.3",
                    "offer": 25,
                    "alcohol": true,
                    "restaurant_opening_time": 16,
                    "restaurant_closing_time": 4
          },
          {
                    "image": "fifteen",
                    "name": "Cafe Bliss",
                    "rating": "1.4",
                    "food_type": "Mexican",
                    "price_for_two": 1789,
                    "location": "Boring Road",
                    "distance": "1.9",
                    "offer": 5,
                    "alcohol": false,
                    "restaurant_opening_time": 14,
                    "restaurant_closing_time": 2
          },
          {
                    "image": "nine",
                    "name": "Hunger Hub",
                    "rating": "4.3",
                    "food_type": "Fast Food",
                    "price_for_two": 246,
                    "location": "Bailey Road",
                    "distance": "5.9",
                    "offer": 27,
                    "alcohol": false,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "six",
                    "name": "Grill House",
                    "rating": "1.8",
                    "food_type": "Mexican",
                    "price_for_two": 2096,
                    "location": "Gandhi Maidan",
                    "distance": "5.0",
                    "offer": 10,
                    "alcohol": false,
                    "restaurant_opening_time": 12,
                    "restaurant_closing_time": 0
          },
          {
                    "image": "nine",
                    "name": "Biryani Express",
                    "rating": "4.9",
                    "food_type": "Mexican",
                    "price_for_two": 733,
                    "location": "Ashok Rajpath",
                    "distance": "6.1",
                    "offer": 9,
                    "alcohol": true,
                    "restaurant_opening_time": 8,
                    "restaurant_closing_time": 20
          },
          {
                    "image": "two",
                    "name": "Cafe Bliss",
                    "rating": "4.9",
                    "food_type": "South Indian",
                    "price_for_two": 584,
                    "location": "Fraser Road",
                    "distance": "5.6",
                    "offer": 4,
                    "alcohol": false,
                    "restaurant_opening_time": 2,
                    "restaurant_closing_time": 14
          },
          {
                    "image": "three",
                    "name": "Street Eats",
                    "rating": "3.7",
                    "food_type": "North Indian",
                    "price_for_two": 1872,
                    "location": "Kurji",
                    "distance": "4.4",
                    "offer": 6,
                    "alcohol": true,
                    "restaurant_opening_time": 12,
                    "restaurant_closing_time": 0
          },
          {
                    "image": "three",
                    "name": "Tandoori Tales",
                    "rating": "3.6",
                    "food_type": "Italian",
                    "price_for_two": 1147,
                    "location": "Bankipore",
                    "distance": "4.9",
                    "offer": 25,
                    "alcohol": false,
                    "restaurant_opening_time": 5,
                    "restaurant_closing_time": 17
          },
          {
                    "image": "three",
                    "name": "Chow Down",
                    "rating": "4.4",
                    "food_type": "Italian",
                    "price_for_two": 1959,
                    "location": "Anisabad",
                    "distance": "9.7",
                    "offer": 5,
                    "alcohol": false,
                    "restaurant_opening_time": 14,
                    "restaurant_closing_time": 2
          },
          {
                    "image": "three",
                    "name": "Dessert Haven",
                    "rating": "4.7",
                    "food_type": "Street Food",
                    "price_for_two": 2216,
                    "location": "Nala Road",
                    "distance": "4.4",
                    "offer": 14,
                    "alcohol": true,
                    "restaurant_opening_time": 19,
                    "restaurant_closing_time": 7
          },
          {
                    "image": "fourteen",
                    "name": "Tandoori Tales",
                    "rating": "2.9",
                    "food_type": "Chinese",
                    "price_for_two": 1021,
                    "location": "Danapur",
                    "distance": "3.7",
                    "offer": 5,
                    "alcohol": true,
                    "restaurant_opening_time": 6,
                    "restaurant_closing_time": 18
          },
          {
                    "image": "fourteen",
                    "name": "Spice Junction",
                    "rating": "1.9",
                    "food_type": "Street Food",
                    "price_for_two": 1418,
                    "location": "Exhibition Road",
                    "distance": "5.8",
                    "offer": 4,
                    "alcohol": false,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "two",
                    "name": "Dessert Haven",
                    "rating": "1.9",
                    "food_type": "Mughlai",
                    "price_for_two": 1989,
                    "location": "Exhibition Road",
                    "distance": "7.6",
                    "offer": 20,
                    "alcohol": false,
                    "restaurant_opening_time": 9,
                    "restaurant_closing_time": 21
          },
          {
                    "image": "four",
                    "name": "Hunger Hub",
                    "rating": "3.0",
                    "food_type": "Thai",
                    "price_for_two": 2228,
                    "location": "Rajendra Nagar",
                    "distance": "6.8",
                    "offer": 6,
                    "alcohol": false,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "three",
                    "name": "Chow Down",
                    "rating": "4.4",
                    "food_type": "Thai",
                    "price_for_two": 524,
                    "location": "Danapur",
                    "distance": "5.6",
                    "offer": 26,
                    "alcohol": false,
                    "restaurant_opening_time": 0,
                    "restaurant_closing_time": 12
          },
          {
                    "image": "ten",
                    "name": "The Curry Bowl",
                    "rating": "4.9",
                    "food_type": "Street Food",
                    "price_for_two": 1133,
                    "location": "Kankarbagh",
                    "distance": "8.4",
                    "offer": 28,
                    "alcohol": true,
                    "restaurant_opening_time": 1,
                    "restaurant_closing_time": 13
          },
          {
                    "image": "ten",
                    "name": "Tandoori Tales",
                    "rating": "3.1",
                    "food_type": "Mexican",
                    "price_for_two": 1674,
                    "location": "Rajiv Nagar",
                    "distance": "2.2",
                    "offer": 18,
                    "alcohol": true,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "five",
                    "name": "Tandoori Tales",
                    "rating": "4.6",
                    "food_type": "Continental",
                    "price_for_two": 860,
                    "location": "S.K. Puri",
                    "distance": "5.3",
                    "offer": 23,
                    "alcohol": false,
                    "restaurant_opening_time": 0,
                    "restaurant_closing_time": 12
          },
          {
                    "image": "thirteen",
                    "name": "The Curry Bowl",
                    "rating": "1.1",
                    "food_type": "Fast Food",
                    "price_for_two": 2020,
                    "location": "Patna City",
                    "distance": "2.1",
                    "offer": 13,
                    "alcohol": false,
                    "restaurant_opening_time": 8,
                    "restaurant_closing_time": 20
          },
          {
                    "image": "one",
                    "name": "Hunger Hub",
                    "rating": "4.5",
                    "food_type": "North Indian",
                    "price_for_two": 2495,
                    "location": "SP Verma Road",
                    "distance": "9.8",
                    "offer": 8,
                    "alcohol": false,
                    "restaurant_opening_time": 11,
                    "restaurant_closing_time": 23
          },
          {
                    "image": "seven",
                    "name": "Cafe Bliss",
                    "rating": "2.3",
                    "food_type": "Fast Food",
                    "price_for_two": 706,
                    "location": "Patna City",
                    "distance": "8.6",
                    "offer": 0,
                    "alcohol": true,
                    "restaurant_opening_time": 18,
                    "restaurant_closing_time": 6
          },
          {
                    "image": "ten",
                    "name": "Chinese Delight",
                    "rating": "3.2",
                    "food_type": "Continental",
                    "price_for_two": 762,
                    "location": "Exhibition Road",
                    "distance": "9.7",
                    "offer": 23,
                    "alcohol": false,
                    "restaurant_opening_time": 7,
                    "restaurant_closing_time": 19
          },
          {
                    "image": "fifteen",
                    "name": "Biryani Express",
                    "rating": "4.4",
                    "food_type": "Thai",
                    "price_for_two": 1629,
                    "location": "Patliputra Colony",
                    "distance": "4.3",
                    "offer": 20,
                    "alcohol": false,
                    "restaurant_opening_time": 16,
                    "restaurant_closing_time": 4
          },
          {
                    "image": "eight",
                    "name": "Biryani Express",
                    "rating": "3.1",
                    "food_type": "Thai",
                    "price_for_two": 884,
                    "location": "SP Verma Road",
                    "distance": "4.7",
                    "offer": 1,
                    "alcohol": false,
                    "restaurant_opening_time": 10,
                    "restaurant_closing_time": 22
          },
          {
                    "image": "twelve",
                    "name": "Cafe Bliss",
                    "rating": "1.1",
                    "food_type": "Mughlai",
                    "price_for_two": 1338,
                    "location": "Bailey Road",
                    "distance": "5.0",
                    "offer": 26,
                    "alcohol": false,
                    "restaurant_opening_time": 8,
                    "restaurant_closing_time": 20
          },
          {
                    "image": "twelve",
                    "name": "Patna Bites",
                    "rating": "3.4",
                    "food_type": "Fast Food",
                    "price_for_two": 215,
                    "location": "Anisabad",
                    "distance": "4.8",
                    "offer": 28,
                    "alcohol": true,
                    "restaurant_opening_time": 9,
                    "restaurant_closing_time": 21
          },
          {
                    "image": "eight",
                    "name": "Tandoori Tales",
                    "rating": "1.1",
                    "food_type": "Continental",
                    "price_for_two": 176,
                    "location": "Nala Road",
                    "distance": "8.2",
                    "offer": 27,
                    "alcohol": false,
                    "restaurant_opening_time": 22,
                    "restaurant_closing_time": 10
          },
          {
                    "image": "fourteen",
                    "name": "Hunger Hub",
                    "rating": "3.5",
                    "food_type": "Italian",
                    "price_for_two": 1063,
                    "location": "Bankipore",
                    "distance": "8.2",
                    "offer": 27,
                    "alcohol": false,
                    "restaurant_opening_time": 19,
                    "restaurant_closing_time": 7
          },
          {
                    "image": "thirteen",
                    "name": "Taste of Punjab",
                    "rating": "5.0",
                    "food_type": "Fast Food",
                    "price_for_two": 247,
                    "location": "Bailey Road",
                    "distance": "4.6",
                    "offer": 10,
                    "alcohol": true,
                    "restaurant_opening_time": 20,
                    "restaurant_closing_time": 8
          },
          {
                    "image": "seven",
                    "name": "The Food Factory",
                    "rating": "2.6",
                    "food_type": "North Indian",
                    "price_for_two": 361,
                    "location": "Patliputra Colony",
                    "distance": "4.4",
                    "offer": 19,
                    "alcohol": false,
                    "restaurant_opening_time": 3,
                    "restaurant_closing_time": 15
          },
          {
                    "image": "eleven",
                    "name": "Hunger Hub",
                    "rating": "1.2",
                    "food_type": "Italian",
                    "price_for_two": 1936,
                    "location": "Ashok Rajpath",
                    "distance": "5.1",
                    "offer": 25,
                    "alcohol": false,
                    "restaurant_opening_time": 11,
                    "restaurant_closing_time": 23
          },
          {
                    "image": "seven",
                    "name": "The Curry Bowl",
                    "rating": "1.3",
                    "food_type": "Fast Food",
                    "price_for_two": 1238,
                    "location": "Boring Road",
                    "distance": "6.3",
                    "offer": 0,
                    "alcohol": false,
                    "restaurant_opening_time": 19,
                    "restaurant_closing_time": 7
          },
          {
                    "image": "fourteen",
                    "name": "Flavors of India",
                    "rating": "1.7",
                    "food_type": "Street Food",
                    "price_for_two": 1609,
                    "location": "Kankarbagh",
                    "distance": "4.0",
                    "offer": 16,
                    "alcohol": true,
                    "restaurant_opening_time": 5,
                    "restaurant_closing_time": 17
          },
          {
                    "image": "fifteen",
                    "name": "Dessert Haven",
                    "rating": "3.2",
                    "food_type": "Street Food",
                    "price_for_two": 1645,
                    "location": "Rajendra Nagar",
                    "distance": "8.4",
                    "offer": 17,
                    "alcohol": false,
                    "restaurant_opening_time": 3,
                    "restaurant_closing_time": 15
          },
          {
                    "image": "fourteen",
                    "name": "Taste of Punjab",
                    "rating": "2.7",
                    "food_type": "Mexican",
                    "price_for_two": 1923,
                    "location": "Bankipore",
                    "distance": "5.7",
                    "offer": 11,
                    "alcohol": false,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "ten",
                    "name": "Street Eats",
                    "rating": "2.1",
                    "food_type": "Street Food",
                    "price_for_two": 881,
                    "location": "Patliputra Colony",
                    "distance": "8.1",
                    "offer": 29,
                    "alcohol": false,
                    "restaurant_opening_time": 5,
                    "restaurant_closing_time": 17
          },
          {
                    "image": "nine",
                    "name": "Flavors of India",
                    "rating": "1.8",
                    "food_type": "Italian",
                    "price_for_two": 2471,
                    "location": "Bailey Road",
                    "distance": "1.2",
                    "offer": 4,
                    "alcohol": false,
                    "restaurant_opening_time": 1,
                    "restaurant_closing_time": 13
          },
          {
                    "image": "thirteen",
                    "name": "Taste of Punjab",
                    "rating": "4.2",
                    "food_type": "South Indian",
                    "price_for_two": 1279,
                    "location": "Danapur",
                    "distance": "7.2",
                    "offer": 13,
                    "alcohol": false,
                    "restaurant_opening_time": 8,
                    "restaurant_closing_time": 20
          },
          {
                    "image": "eleven",
                    "name": "Biryani Express",
                    "rating": "4.5",
                    "food_type": "Chinese",
                    "price_for_two": 171,
                    "location": "Ashok Rajpath",
                    "distance": "7.3",
                    "offer": 17,
                    "alcohol": true,
                    "restaurant_opening_time": 0,
                    "restaurant_closing_time": 12
          },
          {
                    "image": "six",
                    "name": "Grill House",
                    "rating": "2.6",
                    "food_type": "Italian",
                    "price_for_two": 102,
                    "location": "Bankipore",
                    "distance": "1.3",
                    "offer": 10,
                    "alcohol": true,
                    "restaurant_opening_time": 23,
                    "restaurant_closing_time": 11
          },
          {
                    "image": "ten",
                    "name": "Street Eats",
                    "rating": "4.6",
                    "food_type": "Fast Food",
                    "price_for_two": 1001,
                    "location": "Rajendra Nagar",
                    "distance": "8.8",
                    "offer": 9,
                    "alcohol": false,
                    "restaurant_opening_time": 11,
                    "restaurant_closing_time": 23
          },
          {
                    "image": "nine",
                    "name": "Fusion Diner",
                    "rating": "4.4",
                    "food_type": "Mexican",
                    "price_for_two": 527,
                    "location": "Gandhi Maidan",
                    "distance": "3.8",
                    "offer": 9,
                    "alcohol": false,
                    "restaurant_opening_time": 14,
                    "restaurant_closing_time": 2
          }
]

// access all from html
const root = document.querySelector('#root')
function getresturants(resturants) {

          resturants.forEach(resturant => {
                    // create a card
                    const card = document.createElement('div')
                    card.classList.add('bg-white', 'shadow-lg', 'rounded-lg', 'overflow-hidden')

                    // create image
                    const image = document.createElement('img')
                    image.src = `./images/${resturant.image}.jpeg`

                    image.classList.add('w-full', 'h-40', 'object-cover')
                    //       card Details
                    const cardDetails = document.createElement('div')
                    cardDetails.classList.add('p-4')
                    // --------------  create header -----------------------------
                    const header = document.createElement('div')
                    header.classList.add('flex', 'justify-between', 'items-center')

                    const h3 = document.createElement('h3')
                    h3.classList.add('text-lg', 'font-semibold', 'text-gray-800')
                    h3.textContent = resturant.name
                    const rating = document.createElement('span')
                    rating.classList.add('bg-green-500', 'text-white', 'text-sm', 'font-bold', 'px-2', 'py-1', 'rounded')
                    rating.textContent = `★ ${resturant.rating}`
                    header.appendChild(h3)
                    header.appendChild(rating)
                    cardDetails.appendChild(header)

                    //  ------------------------------------- create footer
                    const footer = document.createElement('div')
                    footer.classList.add('flex', 'justify-between', 'items-center', 'mt-2')

                    const food_type = document.createElement('span')
                    food_type.classList.add('text-gray-600', 'text-sm')
                    food_type.textContent = resturant.food_type
                    const price = document.createElement('span')
                    price.classList.add('text-blue-600', 'font-bold')
                    price.textContent = `₹${resturant.price_for_two}`
                    footer.appendChild(food_type)
                    footer.appendChild(price)
                    cardDetails.appendChild(footer)
                    //  ------------------------------------- create location
                    const location = document.createElement('div')
                    location.classList.add('flex', 'justify-between', 'items-center', 'mt-2', 'text-sm')

                    const res_location = document.createElement('span')
                    res_location.classList.add('text-gray-500')
                    res_location.textContent = resturant.location
                    const distance = document.createElement('span')
                    distance.classList.add('text-blue-500')
                    distance.textContent = resturant.distance + 'km'
                    location.appendChild(res_location)
                    location.appendChild(distance)
                    cardDetails.appendChild(location)

                    //                     append all 

                    card.appendChild(image)
                    card.appendChild(cardDetails)
                    root.appendChild(card)
          })
}
getresturants(resturants)

// --------------------------  adding functionality for buttons
// for rating
document.getElementById('ratings').addEventListener('click', () => {
          const data = resturants.filter(resturant => resturant.rating > 4.5)
          root.innerHTML = ''
          getresturants(data)
})
// for alcohol

document.getElementById('alcohol').addEventListener('click', () => {
          const data = resturants.filter(resturant => resturant.alcohol)
          root.innerHTML = ''
          getresturants(data)
})
// for orffer
document.getElementById('offers').addEventListener('click', () => {
          const data = resturants.filter(resturant => resturant.offer > 10)
          root.innerHTML = ''
          getresturants(data)
})
// for filtering
const filter = document.getElementById('filter')
const applyFilter = document.getElementById('applyFilter')
const closeFilter = document.getElementById('closeFilter')
const formFilter = document.getElementById('formFilter')
const popup = document.getElementById('popup')
filter.addEventListener('click', () => {
          popup.classList.remove('hidden');
          popup.classList.add('fixed', 'inset-0', 'bg-gray-800', 'bg-opacity-50', 'flex', 'justify-center', 'items-center')
          console.log("working well...")
})

closeFilter.addEventListener('click', () => {
          popup.classList.add('hidden');
})

applyFilter.addEventListener('click', () => {
          const selectedElement = document.querySelector('input[name="price"]:checked');
          popup.classList.remove('fixed', 'inset-0', 'bg-gray-800', 'bg-opacity-50', 'flex', 'justify-center', 'items-center', 'hidden')
          if (selectedElement.value === 'highToLow') {
                    const data = resturants.sort((a, b) => b.price_for_two - a.price_for_two)
                    root.innerHTML = ''


                    getresturants(data)
          } else if (selectedElement.value === 'lowToHigh') {
                    const data = resturants.sort((a, b) => a.price_for_two - b.price_for_two)
                    root.innerHTML = ''
                    getresturants(data)
          }

          filterForm.reset();
})
// applying search bar

const searchBar = document.getElementById('searchBar')
searchBar.addEventListener('input', () => {
          const value = searchBar.value.toLowerCase()
          const data = resturants.filter(resturant => {
                    return(
                              resturant.name.toLowerCase().includes(value) || 
                              resturant.food_type.toLowerCase().includes(value) || 
                              resturant.location.toLowerCase().includes(value)
                    )                    
          })
          root.innerHTML = ''
          getresturants(data)
})


const hamburgerBtn = document.getElementById('hamburgerBtn');
const mobileMenu = document.getElementById('mobileMenu');

// Toggle mobile menu visibility when hamburger button is clicked
hamburgerBtn.addEventListener('click', () => {
  mobileMenu.classList.toggle('hidden'); // Toggle visibility of mobile menu
});

window.addEventListener('click', (e) => {
            if (!hamburgerBtn.contains(e.target) && !mobileMenu.contains(e.target)) {
              mobileMenu.classList.add('hidden');
            }
          });


          //      funcanality for mobile menu 


          // --------------------------  adding functionality for buttons
// for rating
document.getElementById('mratings').addEventListener('click', () => {
          const data = resturants.filter(resturant => resturant.rating > 4.5)
          root.innerHTML = ''
          mobileMenu.classList.add('hidden');
          getresturants(data)
})
// for alcohol

document.getElementById('malcohol').addEventListener('click', () => {
          const data = resturants.filter(resturant => resturant.alcohol)
          root.innerHTML = ''
          mobileMenu.classList.add('hidden');
          getresturants(data)
})
// for orffer
document.getElementById('moffers').addEventListener('click', () => {
          const data = resturants.filter(resturant => resturant.offer > 10)
          root.innerHTML = ''
          mobileMenu.classList.add('hidden');
          getresturants(data)
})
// for filtering
const mfilter = document.getElementById('mfilter')
mfilter.addEventListener('click', () => {
          mobileMenu.classList.add('hidden');
          popup.classList.remove('hidden');
          popup.classList.add('fixed', 'inset-0', 'bg-gray-800', 'bg-opacity-50', 'flex', 'justify-center', 'items-center')
          console.log("working well...")
})

closeFilter.addEventListener('click', () => {
          popup.classList.add('hidden');
})

applyFilter.addEventListener('click', () => {
          const selectedElement = document.querySelector('input[name="price"]:checked');
          popup.classList.remove('fixed', 'inset-0', 'bg-gray-800', 'bg-opacity-50', 'flex', 'justify-center', 'items-center', 'hidden')
          if (selectedElement.value === 'highToLow') {
                    const data = resturants.sort((a, b) => b.price_for_two - a.price_for_two)
                    root.innerHTML = ''


                    getresturants(data)
          } else if (selectedElement.value === 'lowToHigh') {
                    const data = resturants.sort((a, b) => a.price_for_two - b.price_for_two)
                    root.innerHTML = ''
                    getresturants(data)
          }
         
          filterForm.reset();
})

